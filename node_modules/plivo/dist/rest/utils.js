...
function recursivelyRenameObject(object, renameFunc) {
  ...
  // Don't process non-plain objects like streams, buffers, dates, etc.
  if (!(0, _isPlainObject3.default)(object) && !Array.isArray(object)) {
    return object;
  }
...
function camelCaseRequestWrapper(requestFunc) {
  return function (method, action, params) {
    ...
    params = recursivelyRenameObject(params, function (value, key) {
      ...
      // Skip keys with bracket notation (e.g., documents[0].file) from snake_case transform
      if (/[\[\]]/.test(key)) {
        return key;
      }
