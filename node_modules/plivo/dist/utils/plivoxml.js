...
function Response() {
  ...
  this.nestables = ['Speak', 'Play', 'GetDigits', 'GetInput', 'Record', 'Dial', 'Message', 'Redirect', 'Wait', 'Hangup', 'PreAnswer', 'Conference', 'DTMF', 'MultiPartyCall', 'Stream'];
...
Response.prototype = {
  ...
  addStream: function addStream(body, attributes) {

    var validation = void 0;
    validation = plivoUtils.validateStreamAttributes(body, attributes);
    var item = this;
    if (validation.success == true) {
      var result = item.add(new Stream(Response), body, validation.attributes);
      return result;
    } else {
      throw new Exceptions.PlivoXMLValidationError(validation.msg);
    }
  ...
  toXML: function toXML() {
    return this.elem.toString().replace(/&quot;/g, '"');
  },
...
function Dial(Response) {
  this.element = 'Dial';
  this.valid_attributes = ['action', 'method', 'timeout', 'hangupOnStar', 'timeLimit', 'callerId', 'callerName', 'confirmSound', 'dialMusic', 'confirmKey', 'redirect', 'callbackUrl', 'callbackMethod', 'digitsMatch', 'digitsMatchBLeg', 'sipHeaders', 'confirmTimeout'];
...
function Record(Response) {
  this.element = 'Record';
  this.nestables = [];
  this.valid_attributes = ['action', 'method', 'timeout', 'finishOnKey', 'maxLength', 'playBeep', 'recordSession', 'startOnDialAnswer', 'redirect', 'fileFormat', 'callbackUrl', 'callbackMethod', 'transcriptionType', 'transcriptionUrl', 'transcriptionMethod', 'recordChannelType'];
}
...
function Stream(Response) {
  this.element = 'Stream';
  this.valid_attributes = ['bidirectional', 'audioTrack', 'streamTimeout', 'statusCallbackUrl', 'statusCallbackMethod', 'contentType', 'extraHeaders', 'keepCallAlive'];
  this.nestables = ['break', 'emphasis', 'lang', 'p', 'phoneme', 'prosody', 's', 'say-as', 'sub', 'w'];
}
